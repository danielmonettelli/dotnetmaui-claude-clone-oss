...
public class SimulatedAnthropicService : IAnthropicService
{
    ...
    private readonly List<string> _simulatedResponses =
    [
    ...
    private readonly Dictionary<string, List<string>> _contextResponses = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    private string DetermineResponse(string query)
    {
        ...
        foreach (string context in _contextResponses.Keys)
        {
            if (query.Contains(context, StringComparison.OrdinalIgnoreCase))
            {
                List<string> responses = _contextResponses[context];
                return responses[_random.Next(responses.Count)];
            }
        }
        ...
        {
            string previousMessage = _chatHistory[^2];
